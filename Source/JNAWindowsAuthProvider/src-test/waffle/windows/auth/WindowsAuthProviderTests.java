...
public class WindowsAuthProviderTests extends TestCase {
	...
	/*
	 * public void testLogonGuestUser() { IWindowsAuthProvider prov = new
	 * WindowsAuthProviderImpl(); IWindowsIdentity identity =
	 * prov.logonUser("garbage", "garbage"); debug("Fqn: " + identity.getFqn());
	 * debug("Guest: " + identity.isGuest());
	 * assertTrue(identity.getFqn().endsWith("\\Guest"));
	 * assertTrue(identity.isGuest()); identity.dispose(); }
	 */
