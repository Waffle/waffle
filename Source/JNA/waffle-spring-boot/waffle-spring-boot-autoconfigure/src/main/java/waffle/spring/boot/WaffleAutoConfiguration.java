...
/**
 * Auto configuration for Spring Boot that configures beans
 * based on properties defined in {@link WaffleProperties}.
 */
@Configuration
...
public class WaffleAutoConfiguration {
	...
	/**
	 * The default {@link GrantedAuthority} that is applied to all users.
	 * Default can be overridden by defining a bean of type {@link GrantedAuthority}
	 * with name "defaultGrantedAuthority".
	 */
	@Bean
	...
	/**
	 * The default {@link GrantedAuthorityFactory} that is used. Default
	 * can be overridden by defining a bean of type {@link GrantedAuthorityFactory}.
	 */
	@Bean
	@ConditionalOnMissingBean
	...
	/**
	 * The {@link WindowsAuthenticationProvider} that can be used by Spring Security
	 * by an {@link AuthenticationManager} to provide authentication.
	 * 
	...
	/**
	 * The {@link NegotiateSecurityFilterProvider} that provides single-sign-on
	 * authentication using Negotiate with the configured protocols. Instantiated only
	 * when sso is enabled.
	 * 
	...
	/**
	 * The {@link BasicSecurityFilterProvider} that provides Basic authentication fall back
	 * when using single-sign-on with unsupported browser. Instantiated only when sso is
	 * enabled.
	 * 
	 * @param windowsAuthProvider
	 */
	...
	/**
	 * The {@link SecurityFilterProviderCollection} that includes 
	 * {@link NegotiateSecurityFilterProvider} and/or {@link BasicSecurityFilterProvider}
	 * depending on configuration. Instantiated only when sso is enabled.
	 * 
	...
	public SecurityFilterProviderCollection waffleSecurityFilterProviderCollection(
			NegotiateSecurityFilterProvider negotiateProvider,
			BasicSecurityFilterProvider basicProvider) {
	...
	/**
	 * The {@link NegotiateSecurityFilterEntryPoint} for use by the Spring Security
	 * {@link AuthenticationManager} when using single-sign-on. Instantiated only when
	 * sso is enabled.
	 * 
	 * @param providers
	 */
	...
	/**
	 * The {@link NegotiateSecurityFilter} to be used by Spring Security
	 * {@link AuthenticationManager} when using single-sign-on. Instantiated only when
	 * sso is enabled.
	 * 
	 * @param providers
	...
	/**
	 * When using Spring Boot, {@link Filter}s are automatically registered. In this case,
	 * the filter must be manually configured within an {@link AuthenticationManager}
	 * and so we must prevent Spring Boot from registering it a second time.
	 * 
	 * @param filter The filter that we will be disabling from auto registration.
	 */
	@Bean
	@ConditionalOnProperty("waffle.sso.enabled")
	public FilterRegistrationBean waffleNegotiateSecurityFilterRegistrationBean(
